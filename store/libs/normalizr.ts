import EntitySchema from "./normalizr/EntitySchema";
import IterableSchema from "./normalizr/IterableSchema";
import UnionSchema from "./normalizr/UnionSchema";
import isObject from "lodash/isObject";
import isEqual from "lodash/isEqual";
import mapValues from "lodash/mapValues";

function defaultAssignEntity(normalizedx, key, entity) {
    normalizedx[key] = entity;
}

function getNewObject(schema) {
    if (schema.model) return new schema.model();
    return {};
}
function visitObject(obj, schema, bag, options) {
    const { assignEntity = defaultAssignEntity } = options;

    const normalized = getNewObject(schema);
...
function visitIterable(obj, iterableSchema, bag, options) {
    ...
    const curriedItemMapper = defaultMapper(
        iterableSchema,
        itemSchema,
        bag,
        options
...
function visitEntity(entity, entitySchema, bag, options) {
    ...
    if (!bag[entityKey].hasOwnProperty(id)) {
        bag[entityKey][id] = getNewObject(entitySchema);
    }

    const stored = bag[entityKey][id];
    const normalized = visitObject(entity, entitySchema, bag, options);
...
export function normalize(obj, schema, options = {}) {
    if (!isObject(obj) && !Array.isArray(obj)) {
        throw new Error(
            "Normalize accepts an object or an array as its input."
        );
    }
